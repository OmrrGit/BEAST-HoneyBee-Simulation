...
{
public:
	NeuralMouse(): myBrain(2, 2, 3, false)
	{
		...
		ffnConfig.push_back( 0.0f);
		ffnConfig.push_back( 0.0f); // 2nd hidden 2nd weight (for input 2)
		ffnConfig.push_back( 0.0f); // 2nd hidden bias
		ffnConfig.push_back(-0.5f); // 3rd hidden 1st weight (for input 1)
		ffnConfig.push_back( 0.0f);
		ffnConfig.push_back( 0.0f);
...
public:
	...
	{
		This.Add("angle", NearestAngleSensor<Cheese>());
		This.Add("proximity", ProximitySensor<Cheese>(PI, 80.0, 0.0));
		This.InitRandom = true;
		This.InitFFN(10);
	}
	...
	{
		return This.cheesesFound > 0 ? (static_cast<float>(This.cheesesFound)) / (This.DistanceTravelled.as<float>() * This.MaxTurn.as<float>()) : 0;
	}
